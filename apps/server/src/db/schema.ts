import { pgTableCreator, text, timestamp, boolean, jsonb, unique, index } from 'drizzle-orm/pg-core';
...
export const connection = createTable(
  'connection',
  {
    ...
    providerId: text('provider_id').$type<'google' | 'microsoft' | 'icloud' | 'yahoo'>().notNull(),
    expiresAt: timestamp('expires_at').notNull(),
    createdAt: timestamp('created_at').notNull(),
    updatedAt: timestamp('updated_at').notNull(),
  },
