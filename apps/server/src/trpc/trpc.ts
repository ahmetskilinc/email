...
const activeConnectionProcedure = privateProcedure.use(async ({ ctx, next }) => {
  try {
    const activeConnection = await getActiveConnection(ctx.sessionUser.id);
    return next({ ctx: { ...ctx, activeConnection } });
  } catch (err) {
    await ctx.auth?.api?.signOut({ headers: ctx.c.req.raw.headers }).catch(() => {});
    throw new TRPCError({
      code: 'BAD_REQUEST',
