...
// ---------------------------------------------------------------------------
// getzeitmailDB — returns a per-userId DB interface backed by Postgres
// ---------------------------------------------------------------------------
...
export const getzeitmailDB = async (userId: string) => {
  ...
  return {
    ...
    deleteActiveConnection: (connectionId: string) =>
      db
        .delete(connection)
        .where(and(eq(connection.id, connectionId), eq(connection.userId, userId))),
    ...
    updateConnection: (connectionId: string, data: Partial<typeof connection.$inferInsert>) =>
      db
        .update(connection)
...
// ---------------------------------------------------------------------------
// getActiveConnection — takes an explicit userId instead of Hono context
// ---------------------------------------------------------------------------
...
const resetConnection = async (connectionId: string) => {
  const db = getSharedDb();
  await db
    .update(connection)
